...
import { URL } from 'url'
import { FilterList } from 'cosmetic-filter'
...
const srcsetMax = s => parseSrcset(s).reduce((a, b) => (a.width > b.width || a.density > b.density) ? a : b).url
...
async function preprocess (window,
  ...
  filterList.load(path.join('lib', 'filterlist.txt'))
