...
function preprocessImages (document) {
  ...
  forEachR(document.getElementsByTagName('figure'), figure => {
    ...
    if (images.length > 0) {
      let image = images[0]
      if (!image.hasAttribute('alt') && caption) image.alt = caption.textContent
      if (image.parentNode.tagName.toLowerCase() === 'a') {
        figure.innerHTML = image.parentNode.outerHTML
      } else {
...
async function preprocess (window,
  ...
  article.content = article.content.replace(/<a>(.*?)<\/a>/g, '$1')
